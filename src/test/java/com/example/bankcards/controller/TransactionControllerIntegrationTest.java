...
class TransactionControllerIntegrationTest {
    ...
    @Test
    void cancelTransaction_ShouldCancelTransactionSuccessfullyWhenUserIsSender() throws Exception {
        testTransaction.setStatus(Transaction.TransactionStatus.PENDING);
        when(transactionService.getTransactionById(1L)).thenReturn(testTransaction);
        when(transactionService.cancelTransaction(1L)).thenReturn(testTransaction);
    ...
    @Test
    void cancelTransaction_ShouldReturnForbiddenWhenNotSender() throws Exception {
        User otherUser = User.builder()
                .id(2L)
                .username("otheruser")
                .password("password")
        ...
        when(transactionService.getTransactionById(3L)).thenReturn(otherTransaction);

        mockMvc.perform(post("/api/transactions/3/cancel")
                        .header(HttpHeaders.AUTHORIZATION, "Bearer " + jwtToken))
                .andExpect(status().isForbidden())
